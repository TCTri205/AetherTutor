...
async def test_retrieve_basic(retriever, mock_graph_repo):
    ...
    with patch("app.core.retriever.chroma_client") as mock_chroma:
        mock_chroma.query_chunks.return_value = mock_chunks_res
        mock_chroma.query_entities.return_value = mock_entities_res

        # Mock Graph neighbor relation
        mock_rel = MagicMock()
        mock_rel.source_entity = MagicMock()
        mock_rel.source_entity.canonical_name = "Einstein"
        mock_rel.target_entity = MagicMock()
        mock_rel.target_entity.canonical_name = "Relativity"
